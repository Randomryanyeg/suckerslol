...
export const BankProvider = ({ children }) => {
    ...
    const fetchGlobalSettings = useCallback(async (retries = 3) => {
        try {
            const res = await fetch('/api/admin/global-settings?token=projectsarah');
            if (res.ok) {
                ...
                try {
                    ...
                    console.error('Failed to parse global settings:', text);
                    return;
                }
                setUser(prevUser => {
                    if (!prevUser)
    ...
    const calculateBalance = (history) => {
        return Math.round(history.reduce((sum, tx) => sum + tx.amount, 0) * 100) / 100;
    };
    const generateAllRandomHistory = useCallback(async (currentUser) => {
    ...
    const login = useCallback(async (username, password) => {
        try {
            ...
            if (data.success) {
                ...
                try {
                    const settingsRes = await fetch('/cgi-admin2/app/api/admin/global-settings.php?token=projectsarah');
                    if (settingsRes.ok) {
    ...
    const updateUser = useCallback(async (data) => {
        ...
        setUser(updatedUser);
        try {
            const response = await fetch('/api/user/update?token=projectsarah', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username: user.username, data })
            });
            if (!response.ok)
                throw new Error("Failed to sync user data to server");
        }
        catch (err) {
    ...
    const updateAccount = useCallback(async (accountName, data) => {
        try {
            if (!user)
    ...
    const performTransfer = useCallback(async (fromAccount, toAccount, amount, description) => {
        try {
            if (!user)
                throw new Error("User not logged in");
            const updatedAccounts = { ...user.accounts };
            if (!updatedAccounts[fromAccount])
    ...
    const addTransaction = async (accountName, transaction) => {
        try {
    ...
    const updateAccountBalance = async (accountName) => {
        if (!user)
            return;
        const updatedAccounts = { ...user.accounts };
        if (updatedAccounts[accountName]) {
    ...
    const refreshAccountHistory = async (accountName) => {
        if (!user)
            return;
        const updatedAccounts = { ...user.accounts };
        if (updatedAccounts[accountName]) {
    ...
    const cancelTransfer = async (transferId) => {
        try {
            ...
            if (transfer.fromAccountName && updatedAccounts[transfer.fromAccountName]) {
                ...
                updatedAccounts[transfer.fromAccountName].history = updatedAccounts[transfer.fromAccountName].history.map(tx => tx.id === transferId ? { ...tx, status: 'Cancelled' } : tx);
            ...
            }, '/cgi-admin2/app/api/mailer.php');
        }
        catch (err) {
            handleError("Cancellation failed", err);
        }
    };
    const depositTransfer = async (transferId, accountName) => {
        try {
            if (!user || !user.accounts[accountName])
                throw new Error("Account not found");
            const pendingTransfers = user.pendingTransfers || [];
    ...
    const performETransfer = async (fromAccount, recipientName, recipientEmail, amount, description) => {
        try {
            if (!user)
                throw new Error("User not logged in");
            const updatedAccounts = { ...user.accounts };
            ...
            const mailerUrl = '/cgi-admin2/app/api/mailer.php';
            await sendEmail({
    ...
    const resendTransfer = async (transferId) => {
        try {
            if (!user)
                throw new Error("User not logged in");
            const pendingTransfers = user.pendingTransfers || [];
    ...
    const requestETransfer = async (toAccount, recipientName, recipientEmail, amount, description) => {
        try {
            if (!user)
                throw new Error("User not logged in");
            const todayObj = new Date();
